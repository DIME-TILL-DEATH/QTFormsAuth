    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(&ui_Auth, SIGNAL(loginButtonClicked()), this, SLOT(authorizeUser()));
    connect(&ui_Auth, SIGNAL(destoyed()), this, SLOT(show()));
    connect(&ui_Auth, SIGNAL(authRegisterButtonClicked()), this, SLOT(registerWindowShow()));
    connect(&ui_Reg, SIGNAL(registerButtonClicked()), this, SLOT(registerUser()));
    connect(&ui_Reg, SIGNAL(destroyed()), &ui_Auth, SLOT(show()));

    if(!connectDB()) qDebug() << "Failed to connect DB";
    ...
    if(!query.exec(db_input)) qDebug() << "Unable to create a table" << query.lastError();
...
void MainWindow::authorizeUser()
{
    ...
    if(!query.exec(db_input)) qDebug() << "Unable to execute query - exiting" << query.lastError() << " : " << query.lastQuery();
